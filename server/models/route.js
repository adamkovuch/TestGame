"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
var Route_1;
const typegoose_1 = require("typegoose");
const db_connection_1 = require("../core/db.connection");
const localization_1 = require("../core/localization");
let Route = Route_1 = class Route extends typegoose_1.Typegoose {
};
__decorate([
    typegoose_1.prop({ required: true }),
    localization_1.lang(),
    __metadata("design:type", Object)
], Route.prototype, "name", void 0);
__decorate([
    typegoose_1.prop({ required: true, unique: true }),
    __metadata("design:type", String)
], Route.prototype, "alias", void 0);
__decorate([
    typegoose_1.prop({ required: true, default: false }),
    __metadata("design:type", Boolean)
], Route.prototype, "isDefault", void 0);
__decorate([
    typegoose_1.prop({ required: true, default: 0 }),
    __metadata("design:type", Number)
], Route.prototype, "position", void 0);
__decorate([
    typegoose_1.prop({ required: true, default: true }),
    __metadata("design:type", Boolean)
], Route.prototype, "visible", void 0);
__decorate([
    typegoose_1.prop({ required: true }),
    __metadata("design:type", String)
], Route.prototype, "component", void 0);
Route = Route_1 = __decorate([
    typegoose_1.post('remove', (route) => {
    }),
    typegoose_1.pre('save', (next) => {
        if (this.isDefault === true) {
            const RouteModel = new Route_1().getModelForClass(Route_1);
            RouteModel.update({ 'isDefault': true }, { 'isDefault': false });
        }
        next();
    })
], Route);
exports.Route = Route;
db_connection_1.DbModels.registerModel(Route);
//# sourceMappingURL=route.js.map